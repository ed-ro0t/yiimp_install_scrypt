...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    echo "phpmyadmin:$basicauthphpmyadmin" | sudo tee /etc/nginx/pma_pass_phpmyadmin
...
echo 'include /etc/nginx/blockuseragents.rules;
	...
	if ($blockedagent) {
 	...
 	client_body_buffer_size  50k;
		...
		location /phpmyadmin {
        auth_basic "Admin Login";
        auth_basic_user_file /etc/nginx/pma_pass_phpmyadmin;
  		root /usr/share/;
  		index index.php;
  		try_files $uri $uri/ =404;
...
[pwdbasicauthphpmyadmin]
user=phpmyadmin
password='"${pwdbasicauthphpmyadmin}"'
[mysql]
user=root
password='"${rootpasswd}"'
